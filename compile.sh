...
cmake -B build_leaky \
   ...
   -D pgen=mcp -D precision=double -D mpi=OFF -D output=ON -D ADIOS2_USE_CUDA=OFF -D Kokkos_ENABLE_CUDA=ON -D gpu_aware_mpi=OFF -D Kokkos_ARCH_VOLTA70=ON \
   -D deposit=esirkepov -D shape_order=9
cmake --build build_leaky -j
