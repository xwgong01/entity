  ...
  struct PGen {
	...
	// reset status
      ...
      if constexpr (D == Dim::_2D) {
		...
		// TODO: use dx.
	        for (auto d : {0,1}){
		  e_interp[d] = e_interp[d] * 0.2;
		  b_interp[d] = b_interp[d] * 0.2;
		}


                real_t dwxpar, dwypar, dwzpar, dwxperp, dwyperp, dwzperp;
		real_t gm = math::sqrt(u1(p)*u1(p)+u2(p)*u2(p)+u3(p)*u3(p)+1);
		real_t bnorm = math::sqrt(b_interp[0]*b_interp[0]+b_interp[1]*b_interp[1]+b_interp[2]*b_interp[2]);
	
		// already normalized
		b_interp[0] = b_interp[0] / bnorm;
		b_interp[1] = b_interp[1] / bnorm;
		b_interp[2] = b_interp[2] / bnorm;
		
		real_t Epar = (e_interp[0]*b_interp[0]+e_interp[1]*b_interp[1]+e_interp[2]*b_interp[2]);


		dwxpar = u1(p) / gm * Epar *b_interp[0] ;
		dwypar = u2(p) / gm * Epar *b_interp[1] ;
		dwzpar = u3(p) / gm * Epar *b_interp[2] ;
		dwxperp = u1(p) / gm * (e_interp[0] - Epar * b_interp[0]);
		dwyperp = u2(p) / gm * (e_interp[1] - Epar * b_interp[1]);
		dwzperp = u3(p) / gm * (e_interp[2] - Epar * b_interp[2]);
